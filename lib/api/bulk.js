...
Bulk.prototype.query = function (soql, options) {
  ...
  return new Promise((resolve, reject) => {
    this.load(type, 'query', options || {}, soql).then((results) => {
      const lazyLoadedResultStreams = results.map((result) => {
        const batch = self.job(result.jobId).batch(result.batchId);

        return function processStream (nextStreams) {
          return new Promise((resolveStream, rejectStream) => {
            ...
            const stream = batch
              ...
              .stream()
              .on('end', () => {
                console.info('Finished streaming results from batch:', result.batchId);
                if (nextStreams.length > 0)
                  nextStreams.shift()(nextStreams).then(() => { resolveStream(); });
                else
                  resolveStream();
              })
              .on('error', (err) => {
            ...
            // Pipe the stream to the dataStream
            stream.pipe(dataStream, { end: false });
          }
        });
      });

      if (lazyLoadedResultStreams.length > 0)
        return lazyLoadedResultStreams.shift()(lazyLoadedResultStreams).then(() => {
          dataStream.end();
          resolve();
        }).catch((err) => {
